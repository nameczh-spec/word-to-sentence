...
### 已完成（2026-07-05更新）
- [x] **作文范文整合**：42/42套试卷作文范文完整（含Part A/Part B、范文、技巧、结构提示）
- [x] **2012-1 Part B图片补充**：从新东方网站下载，同步3目录
- [x] **翻译analysis补充**：2000/2001/2019-1年全部填充，2005/2011年同步修正md和JSON
- [x] **2005年作文核对**：Part A辞职信和Part B养老"足球赛"正确
- [x] **2011年作文修正**：Part B从"倒水/关爱"修正为"旅途之'余'"旅游环保主题
- [x] **真题精读撤销功能修复**：绘制前保存Canvas状态到撤销栈
- [x] **完形填空清除按钮修复**：清除画布涂鸦而非答题选项
- [x] **真题模块移动端响应式样式**：pastPapers.css新增768px和480px断点
- [x] **主页面移动端适配优化**：main.css新增768px和480px断点
- [x] **移动端UI第二阶段优化（10项）**：
  - 汉堡菜单样式修复（36×36px触控区，.nav-hamburger）
  - 侧边栏历史记录入口图标样式修复（.sidebar-history-entry-btn）
  - 点击空白区域侧边栏自动关闭（sidebar-overlay.active遮罩层）
  - 手机端下拉框左侧空白过大修复（480px断点，label固定70px）
  - Pad端下拉框长短不一修复（新增1024px断点，垂直排列）
  - Pad端按钮显示不全修复（1024px断点优化按钮布局和间距）
  - 设置面板复习历史图标过大修复（36px图标，.review-entry-svg-icon）
  - 移动端隐藏拓展功能（.desktop-only类，≥901px显示）
  - 移动端导航栏添加真题练习按钮（.nav-pastpapers-btn）
  - 设置面板账号名称换行修复（white-space: nowrap）
...
- [x] **项目清理**：删除228个临时文件和冗余目录
- [x] **数据修复**：补充2006年真题数据（完形填空、阅读理解Part A/B、翻译、作文）
- [x] **数据修复**：补充2000年和2001年翻译部分（从网上获取参考译文）
- [x] **Bug检查**：getThemeColors错误未在代码中发现，设置面板CSS默认隐藏正常
- [x] **移动端真题模块移植**：复制6个JS+1个CSS+数据目录+图片到移动端
- [x] **密钥文件安全处理**：将miyao/目录密钥文件移至项目外部
- [x] **移动端界面适配**：
  - 修复平板端(768-900px)参数区域横向布局，生成引擎和输出格式下拉框并排显示
  - 修复移动端单词操作按钮不可见问题（触控设备无hover，改为始终显示）
  - 修复tab-btn类名错误（原.group-tab应改为.tab-btn）
  - 修复辅助按钮组溢出（添加flex-wrap）
  - 修复释义弹窗筛选栏溢出（添加flex-wrap）
  - 增大导航栏按钮、弹窗关闭按钮触控区域至40px/36px
  - 修复iOS输入框自动缩放问题（font-size设为16px）
  - 释义弹窗改为底部抽屉式（与其他弹窗风格统一）
- [x] **移动端界面继续优化**：
  - 小屏幕文字可读性优化（9-11px文字增大到12px）
  - 释义勾选框触控区域增大（15px→32px）
  - 复制提示词图标按钮增大（16px→32px）
  - 词本列表项操作按钮增大到36px
  - 设置面板按钮布局均分
  - 历史记录列表高度增大到120px
- [x] **翻译和作文内容整合**：
  - 解压语料库mimoclaw_workspace.tar.gz（42个md文件）
  - 整合翻译译文和解析到120个JSON文件
  - 整合作文范文和写作技巧到JSON文件

### 待完成（按优先级排序）

#### 优先级1：数据校验与完善
- [ ] **翻译数据全量校验**：42套JSON的translation和analysis，当前仅抽查了问题年份，需逐条验证完整性
- [ ] **作文范文全量校验**：42套writing.js的Part A/Part B，确认范文/技巧/图片引用完整

#### 优先级2：移动端剩余优化
- [ ] 移动端真题页面UI适配测试（真机或Chrome设备模拟器）
- [ ] 小屏幕(<375px)文字可读性进一步优化
- [ ] GitHub待推送（本地有两次提交未推送至远程仓库）

#### 优先级3：不背单词风格功能开发
- [x] 创建learningEngine.js基础架构（含记忆曲线算法）
- [x] 实现记忆曲线算法（艾宾浩斯+简易+挑战三种模式）
- [x] 创建沉浸式深色主题CSS（vocab-learning.css）
- [x] 基础学习页面UI（首页仪表盘+单词本+设置三栏）
- [x] 翻卡复习模式（认识/不认识、进度条、结果统计）
- [x] 学习数据管理（导出/导入/重置）
- [x] Web端+移动端同步集成
- [x] **实现默写练习**（键盘输入+手写输入Canvas+橡皮擦/撤销/清空）
- [x] 默写设置页面（输入方式/默写模式/数量/词源选择）
- [x] 默写结果页面（正确率/用时/错误单词列表）
- [x] **拼写练习**：看释义拼写单词，编辑距离模糊匹配
- [x] **语境选择**：4选1完形填空，错误时高亮正确答案
- [x] **单词配对**：双栏点击配对，支持自动完成检测
- [x] **词典功能**：词根词缀库(60+)、派生词关联、柯林斯英英释义
- [x] 词典深色弹窗（毛玻璃遮罩+四个分区展示）
- [x] **Pad端适配**（769-1024px断点优化）
- [x] 桌面端/平板/手机端三级断点
...
### 6.3 已知问题
1. ~~**2006年真题数据完全缺失**~~：已修复，已补充2006.json和index.json条目
2. **2套试卷翻译缺失**：需运行检查脚本确定具体是哪两套
3. **getThemeColors错误**：控制台报错 `Cannot destructure property 'exportedColors' of 'undefined'`（代码中未发现，可能已修复）
4. **设置面板默认打开**：页面加载后设置面板应默认隐藏（CSS已设置默认隐藏）
5. ~~**移动端真题模块缺失**~~：已修复，已复制6个JS+1个CSS+数据目录+图片
6. ~~**移动端密钥明文存储**~~：已修复，密钥文件已移至项目外部
