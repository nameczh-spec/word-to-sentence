...
const DataStore = {
    ...
    archiveToday() {
        ...
        // 【星标控制】仅归档已收藏的页面
        const starredPages = currentText && currentText.starredPages;
        const hasStarred = starredPages && starredPages.some(s => s);
        if (hasStarred) {
            // 只提取已收藏页面的原文
            const rawParagraphs = (currentText.rawContent || '').split(/\n\n+/);
            const starredContent = starredPages
                .map((starred, index) => (starred ? rawParagraphs[index] : null))
                .filter(p => p && p.trim())
                .join('\n\n');
            ...
            console.log('已归档收藏页面到历史记录');
        } else if (currentText && currentText.text) {
