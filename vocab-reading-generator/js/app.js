    ...
    bindMainEvents() {
        ...
        if (apiKeyToggleBtn && apiKeyInput) {
            apiKeyToggleBtn.addEventListener('click', () => {
                ...
                const eyeOpen = apiKeyToggleBtn.querySelector('.eye-open');
                const eyeClosed = apiKeyToggleBtn.querySelector('.eye-closed');
...
// 页面关闭前自动归档（仅当天模式，且确保归档纯净数据）
window.addEventListener('beforeunload', () => {
    // 【数据隔离】只读模式下禁止归档，防止历史数据污染当日存储
    if (window.WordManager && !WordManager._isReadOnly) {
        // 归档前强制从 localStorage 重新加载当日词单，确保数据纯净
