...
(function () {
    ...
    var LocalGenerator = {
        ...
        generate: function (wordsArray, mode) {
            ...
            var selectedTopics;
            if (mode === 'chinese') {
                // 中文模式：不使用按主题分组，直接从中文骨架池选取
                selectedTopics = ['chinese'];
            } else {
                // 随机选2~3个主题（每个主题生成一段）
                selectedTopics = this._pickRandom(topics, Math.min(3, topics.length));
            }
            ...
            for (var ti = 0; ti < selectedTopics.length; ti++) {
                var topic = selectedTopics[ti];
                var topicSkeletons;
                if (mode === 'chinese') {
                    // 中文模式：获取所有中文骨架（含内置保底）
                    topicSkeletons = this._getChineseSkeletons();
                    // 如果没有中文骨架，回退到英文模式
                    if (topicSkeletons.length === 0) {
                        topicSkeletons = this._getSkeletonsByTopic('general');
                    }
                } else {
                    topicSkeletons = this._getSkeletonsByTopic(topic);
                }
                // 如果主题骨架为空，跳过该主题
                if (topicSkeletons.length === 0) continue;
                // 从该主题选 3~5 个骨架
                var count = Math.min(3 + Math.floor(Math.random() * 3), topicSkeletons.length);
                var selected = this._pickRandom(topicSkeletons, count);

                var sentences = [];
                for (var si = 0; si < selected.length; si++) {
                    var skel = selected[si];
                    var result = this._fillSkeleton(skel, userByPos, mode);
                ...
                // 用过渡词连接句子
                var paragraph = this._assembleParagraph(sentences, ti, selectedTopics.length, mode);
         ...
         */
        _fillSkeleton: function (skeleton, userByPos, mode) {
            ...
            for (var i = allMatches.length - 1; i >= 0; i--) {
                ...
                if (type === 'num') {
                    ...
                    replacement = this._pickSemanticWord(userByPos, 'Name', usedWords);
                } else if (type === 'term') {
                    // {term} → 专业术语：优先选较长单词（≥8字母）
                    replacement = this._pickSemanticWord(userByPos, 'term', usedWords);
                } else if (type === 'n' || type === 'v' || type === 'adj' || type === 'adv') {
                    // 标准词性
                    formCount[type]++;
                    var form = formMap[type][formCount[type]] || 'base';
                    var word = this._pickWord(userByPos, type);
         ...
         */
        _pickSemanticWord: function (userByPos, semanticType, usedWords) {
            var candidates = userByPos.n || [];
            var filtered = [];

            // 过滤掉已使用的词（不修改原数组，仅过滤）
            for (var i = 0; i < candidates.length; i++) {
                if (usedWords.indexOf(candidates[i].toLowerCase()) === -1) {
         ...
         * @returns {string|null}
         */
        _pickWord: function (userByPos, pos) {
            var candidates = userByPos[pos];
            if (candidates && candidates.length > 0) {
                // 随机选一个
                var idx = Math.floor(Math.random() * candidates.length);
                // 移除已选词（避免同一段落重复使用）
                var word = candidates.splice(idx, 1)[0];
                return word;
            }
            return null;
        },
