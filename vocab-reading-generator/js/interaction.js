...
const InteractionManager = {
    ...
    async _showNonImportedWordPopup(word, e) {
        ...
        document.getElementById('cardTranslation').textContent = '查询中...';
        
        // 位置跟随鼠标
        const rect = e.target.getBoundingClientRect ? e.target.getBoundingClientRect() : { bottom: e.clientY, left: e.clientX };
        ...
        try {
            ...
            if (typeof VocabSearch !== 'undefined') {
                // 调用有道 API 获取释义（await 等待完成后直接使用数据）
            ...
            // API失败或不可用，显示简单提示
            document.getElementById('cardPos').textContent = '';
            document.getElementById('cardTranslation').textContent = `"${word}" 查询失败，请检查网络或API配置`;
            setTimeout(() => this.hideSimpleCard(), 2500);
        } catch (e) {
            console.error('非导入单词查询失败:', e);
            document.getElementById('cardPos').textContent = '';
            document.getElementById('cardTranslation').textContent = '查询失败';
            setTimeout(() => this.hideSimpleCard(), 2000);
        }
    },
        ...
        if (options.fromClick && options.youdaoData) {
            ...
            if (entries.length > 0) {
                ...
                if (parsedItems.length > 0) {
                    ...
                    // 构建HTML：每张卡片显示一个词性，所有释义按Youdao顺序展示
    ...
    initSourceToggle() {
        ...
        toggle.addEventListener('click', (e) => {
            const btn = e.target.closest('.source-toggle-btn');
            if (!btn) return;
            ...
            if (source === 'custom') {
                if (!this._selectedCustomBook) {
                    // No book selected yet - auto-open book manager
